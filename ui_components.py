...
def create_folder_selection(frame, controller, select_folder_callback):
    tk.Label(frame, text=controller._("Source folder path:"), font=("Arial", 12)).grid(row=0, column=0, padx=5, pady=5, sticky="w")
...
def create_format_selection(frame, controller, label_text, row, callback):
    tk.Label(frame, text=controller._(label_text), font=("Arial", 12)).grid(row=row, column=0, padx=5, pady=5, sticky="w")
...
def create_rename_direction_selection(frame, controller, callback):
    tk.Label(frame, text=controller._("Rename direction:"), font=("Arial", 12)).grid(row=3, column=0, padx=5, pady=5, sticky="w")
...
def create_rename_button(frame, controller, callback):
    tk.Button(frame, text=controller._("Rename files"), command=callback,
              font=("Arial", 10, "bold"), bg="#4CAF50", fg="#FFFFFF").grid(row=5, column=0, columnspan=3, padx=5, pady=5, sticky="e")